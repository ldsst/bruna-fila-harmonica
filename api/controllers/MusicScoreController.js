...
class MusicScoreController {
    ...
    async createMusicScore(req, res) {
        ...
        try {
            const newMusicScoreCreated = await database.Partitura.create(newMusicScore)
            return res.status(200).json(newMusicScoreCreated)
        } catch (error) {
            return res.status(500).json(error.message)
        }
    }
    ...
    async updateMusicScore(req, res) {
        ...
        try {
            ...
            const updatedMusicScore = await database.Partitura.findOne({ where: { idPartitura: Number(id) } })
