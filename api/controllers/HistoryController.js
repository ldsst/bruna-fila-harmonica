...
class HistoryController {
  ...
  async createHistory(req, res) {
    ...
    try {
      await database.Historico.create(newHistory)
      return res.status(201).json({ message: 'History was created successfully' })
    } catch (error) {
      return res.status(500).json(error.message)
    }
  }
  ...
  async updateHistory(req, res) {
    ...
    try {
      ...
      const updatedHistory = await database.Historico.findOne({
        where: { idHistorico: Number(id) }
      })
